...
private extension Publishers.Create {
    final class Subscription<Downstream: Combine.Subscriber>: ThreadSafeCancellable, Combine.Subscription
        where
        ...
        init(
            ...
            let subscriber = Subscriber(
                onValue: { [weak self] value in
                    ...
                    let buffer = withCancellationLock { self.buffer }
...
public extension Publishers.Create {
    struct Subscriber {
        ...
        ///
        /// - Parameter completion: A `Completion` instance which indicates whether publishing has finished normally or failed
        /// with an error.
