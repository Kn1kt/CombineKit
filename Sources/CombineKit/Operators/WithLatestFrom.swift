...
public extension Publisher {
    ...
    func withLatestFrom<Other: Publisher, Other1: Publisher, Other2: Publisher>(
        ...
        Self,
        AnyPublisher<(Other.Output, Other1.Output, Other2.Output), Self.Failure>,
        (Other.Output, Other1.Output, Other2.Output)
    >
    where
        Other.Failure == Failure,
        Other1.Failure == Failure,
...
public extension Publishers {
    ...
    >: Publisher
    where
        Upstream.Failure == Other.Failure
    {
...
private extension Publishers.WithLatestFrom {
        ...
        Downstream.Failure == Failure
    {
        ...
        private let latestValueLock: some Locking = Locks.default
        private var latestValue: Other.Output?
        ...
        init(
            ...
            trackLatestFromSecond { [weak self] in
                ...
                let preInitialDemand = latestValueLock.withLock {
        ...
        func request(_ demand: Subscribers.Demand) {
            let needToRequestDemand = latestValueLock.withLock {
                if latestValue == nil {
            ...
            guard needToRequestDemand else { return }
        ...
        private func trackLatestFromSecond(onInitialValue: @escaping () -> Void) {
            ...
            let subscriber = AnySubscriber<Other.Output, Other.Failure>(
                receiveSubscription: { [weak self] subscription in
                    ...
                    if withCancellationLock({ self.otherSubscription = subscription }) != nil {
                ...
                receiveValue: { [weak self] value in
                    ...
                    latestValueLock.withLock { self.latestValue = value }
                    ...
                    if !gotInitialValue {
                        ...
                        let sink = Sink(
                            ...
                            transformOutput: { [weak self] value in
                                guard let self,
                                      let other = latestValueLock.withLock({ self.latestValue }) else {
                                    return nil
                                }
                        ...
                        if withCancellationLock({ self.sink = sink }) != nil {
        ...
        override func cancel() {
            withCheckedCancellation {
                sink.kill()
                otherSubscription.kill()
            }
        }
    }
}

extension Publishers.WithLatestFrom.Subscription: CustomStringConvertible {
    var description: String {
        return "WithLatestFrom.Subscription<\(Output.self), \(Upstream.Failure.self)>"
    }
}
